...
/obj/machinery/ticket_machine/proc/handle_maptext()
	switch(current_number) //This is here to handle maptext offsets so that the numbers align.
		...
		if(100)
			maptext_x = 4
	maptext = MAPTEXT_MNC(current_number) //Finally, apply the maptext  NAAKAS-LOUNGE EDIT: MNC Maptext
...
/obj/item/ticket_machine_ticket/Initialize(mapload, num)
	...
	if(!isnull(num))
		name += " #[num]"
		saved_maptext = MAPTEXT_MNC(num) /// NAAKAS-LOUNGE EDIT: MNC Maptext
